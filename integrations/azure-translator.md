...
authors:
    - name: recrudesce (Russ)
      socials:
        github: recrudesce
        twitter: recrudesce
pypi: https://pypi.org/project/deepstack-translate-node/ 
repo: https://github.com/recrudesce/deepstack_translate_node
type: Custom Component
report_issue: https://github.com/recrudesce/deepstack_translate_node/issues
---
...
## Installation
Run `pip install deepstack-translate-node` to install the latest available version.

## Usage
Include in your pipeline as follows:

```python
from deepstack_translate_node import TranslateAnswer, TranslateQuery
...
You can then query your `base_lang` corpus using the `query` value as normal using a standard Deepstack Retriever node, which will place your results in `results`.
