...
layout: integration
name: langfuse
description: Monitor and trace your Deepstack requests.

authors:
    - name: khulnasoft
      socials:
        github: khulnasoft
        twitter: deepstack_ai
        linkedin: khulnasoft
pypi: https://pypi.org/project/langfuse-deepstack/
repo: https://github.com/khulnasoft/deepstack-core-integrations/tree/main/integrations/langfuse
...
```bash
pip install langfuse-deepstack
```
...
- The [`LangfuseConnector`](https://docs.deepstack.khulnasoft.com/docs/langfuseconnector): 
  
    `LangfuseConnector` connects Deepstack LLM framework with Langfuse in order to enable the tracing of operations
    ...
    In addition, you need to set the `DEEPSTACK_CONTENT_TRACING_ENABLED` environment variable to `true` in order to
    enable Deepstack tracing in your pipeline.

    These code examples also require an [`OPENAI_API_KEY`](https://help.openai.com/en/articles/4936850-where-do-i-find-my-openai-api-key) environment variable to be set. Deepstack is model-agnostic and you can [use any model provider we support](https://docs.deepstack.khulnasoft.com/docs/generators), by changing the generator in the code samples below.
...
`langfuse-deepstack` is distributed under the terms of the [Apache-2.0](https://spdx.org/licenses/Apache-2.0.html) license.
