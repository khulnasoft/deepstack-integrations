...
authors:
    - name: Tuana Çelik
      socials:
        ...
        twitter: tuanacelik
        linkedin: tuanacelik
pypi: https://pypi.org/project/mastodon-fetcher-deepstack/
repo: https://github.com/tuanacelik/mastodon-fetcher-deepstack
type: Data Ingestion
report_issue: https://github.com/tuanacelik/mastodon-fetcher-deepstack/issues
...
You can see a demo of this custom component in the [🦄 Should I Follow?](https://huggingface.co/spaces/khulnasoft/should-i-follow) space on Hugging Face 🤗.

The latest versions of `mastodon-fetcher-deepstack` are compatible only with Deepstack 2.x. You need to specify the version explicitly to import the `MastodonFetcher` component suitable with Deepstack 1.x.
...
### Installation (2.0)
```bash
pip install mastodon-fetcher-deepstack
```
...
#### On its own:
```python
from mastodon_fetcher_deepstack.mastodon_fetcher import MastodonFetcher
...
```bash
pip install mastodon-fetcher-deepstack==0.0.1
```
